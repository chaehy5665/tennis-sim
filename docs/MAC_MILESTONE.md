...
2. 흐림: 행 이름, 머리글, 제목, Badge는 흐리지 않는지. "참고용" 태그가 붙은 패널은 값(SplitBar 글자, 표 값, 아래 줄)이
   모두 흐린지. 비교 표도 태그가 붙으면 "이번" 값까지 흐린지.
...
5. 공격 방향 표의 "그 외" 행: 둘째 줄 "빈 곳 · 강타 · 깊게"(12px, 흐림). 행 높이는 항목 이름이 body라 약 50px다
   (디자인 44px는 label 기준, Design에 확인 요청함).
